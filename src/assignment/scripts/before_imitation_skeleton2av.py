...
human_joint_dict = {"head":0, "neck":1, "torso":2, "left_shoulder":3,
                    "left_elbow":4, "left_hand":5, "left_hip":6, "left_knee":7, "left_foots":8,
...
def make_simple_motion():
    ...
    av["RARM-SHOULDER-R"] = -90 - np.rad2deg(np.arctan2(left_arm_dir[2], left_arm_dir[0]))
    av["RARM-SHOULDER-P"] = 90 + np.rad2deg(np.arctan2(left_arm_dir[2], left_arm_dir[1]))
    
    av["LARM-SHOULDER-R"] = 90 + np.rad2deg(np.arctan2(right_arm_dir[2], right_arm_dir[0]))
    av["LARM-SHOULDER-P"] = 90 + np.rad2deg(np.arctan2(right_arm_dir[2], right_arm_dir[1]))

def make_av_manually():
    joint = "LARM-SHOULDER-R"
    angle = 60

    av[value2key(joint)] = angle
